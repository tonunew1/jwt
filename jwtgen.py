...
def get_token(password, uid, max_retries=5, retry_delay=13):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if "access_token" in token_json and "open_id" in token_json:
                    ...
                    print(f"[Retry {attempt+1}/{max_retries}] Invalid token response, retrying...")
            else:
                print(f"[Retry {attempt+1}/{max_retries}] HTTP {res.status_code} - Retrying...")
...
def get_single_response():
    ...
    if not token_data:
        # After 20 retries, still no valid token
        return jsonify({
            "uid": uid,
            "status": "invalid",
